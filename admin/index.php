...
if ( isset( $_POST['gemini_ayarlari_kaydet'] ) ) {
    ...
    // --- CRON GÜNCELLEME MANTIĞI ---
    // Ayar değiştiği için eski zamanlayıcıyı silip yenisini kuruyoruz
    wp_clear_scheduled_hook( 'gemini_gorevi_v5' );
    
    // Eğer daily ise gece yarısında (00:00) çalıştır, değilse normal zamanla
    if ( $yeni_aralik === 'daily' ) {
        // WordPress timezone'una göre gece yarısı zamanını hesapla
        wp_schedule_event( gemini_gece_yarisi_zamani(), $yeni_aralik, 'gemini_gorevi_v5' );
    } else {
...
// Mevcut ayarı veritabanından çek (Varsayılan: günlük - gece yarısı)
            ...
            <form method="post" action="">
                ...
                <div class="mb-3">
                    ...
                    <select name="gemini_cron_aralik" id="zamanlama" class="form-select">
                        ...
                        <option value="daily" <?php selected( $mevcut_aralik, 'daily' ); ?>>Günde Bir - Gece Yarısı (00:00)</option>
                        <option value="weekly" <?php selected( $mevcut_aralik, 'weekly' ); ?>>Haftada Bir (Weekly)</option>
                    </select>
                    <div class="form-text text-muted">
                        Seçtiğiniz aralıkta konular dosyasından bir satır silinir ve taslak oluşturulur. "Günde Bir" seçeneği WordPress timezone ayarınıza göre her gün gece yarısı (00:00) çalışır.
                    </div>
                </div>
    ...
    <div class="card mt-4 border-info">
        <div class="card-body">
            ...
            <ul class="list-group list-group-flush">
                ...
                <?php if ( $mevcut_aralik === 'daily' ): ?>
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    WordPress Timezone:
                    <span class="badge bg-info rounded-pill"><?php 
                        $tz = get_option( 'timezone_string' );
                        if ( empty( $tz ) ) {
                            $gmt_offset = get_option( 'gmt_offset' );
                            echo 'UTC' . ( $gmt_offset >= 0 ? '+' : '' ) . $gmt_offset;
                        } else {
                            echo $tz;
                        }
                    ?></span>
                ...
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    Sonraki Çalışma:
                    <?php 
                    $next_run = wp_next_scheduled( 'gemini_gorevi_v5' );
                    if ( $next_run ) {
                        $next_run_local = date_i18n( 'Y-m-d H:i:s', $next_run );
                        echo '<span class="badge bg-success rounded-pill">' . $next_run_local . '</span>';
                    } else {
                        echo '<span class="badge bg-warning rounded-pill">Zamanlanmamış</span>';
                    }
                    ?>
                </li>
                <?php endif; ?>
